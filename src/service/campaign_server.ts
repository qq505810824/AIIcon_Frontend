...
export const getRandomApps = async (options?: any) => {
    try {
        ...
        let query = supabase.rpc('random_Campaigns', options);
        const { data, error } = await query;
        if (error) {
            throw error;
        }
...
export const updateApp = async (id: number, appData: Partial<CampaignModel>) => {
    try {
        let result;
        // 如果是更新 focus，使用 RPC
        if ('focus' in appData) {
...
export const deleteApp = async (id: number, community_id?: number) => {
    try {
        if (community_id) {
            const tasks = [
                supabase.rpc('decrement_community_Campaign', { community_id: community_id }),
